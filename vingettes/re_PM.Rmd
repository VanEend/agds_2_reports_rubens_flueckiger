...
To reduce the length of this file I saved functions created for this exercise into a separate R file which I will load in with all necessary packages used.
...
source("../functions/Phenology_modelling_functions.R")
```
...
In a first step we will acquire the Phenocam data set of the Harvard data set. To improve the loading speed I saved the data set into the *data* folder and used a if(FALSE) string to prevent the code to re-upload the data set from the Phenocam server.
...
if(FALSE){ #Prevents re-load
  download_phenocam(
    ...
    phenophase = TRUE,
    trim = 2022,
    out_dir = "../data/Phenology_modelling/" #Store data set in folder
  )
}
```
...
harvard_data <- read.table(
  "../data/Phenology_modelling/harvard_DB_1000_3day.csv",
  header = T, sep = ",")

# 2. Day of leave out information
harvard_phenology <- read.table(
  "../data/Phenology_modelling/harvard_DB_1000_3day_transition_dates.csv",
  header = T, sep = ","
)
```
...
First we will acquire the MCD12Q2 data set. I used the if(FALSE) statement again to prevent a re-acquire of the data set, since I stored the needed files in the *data* folder. As I encountered some difficulties to plot the MCD12Q2 file on a netcdf4 format, I decided to store the the file in a geotiff format. Since both formats work with the `terra` package we do not have to worry about further complications.
...
if(FALSE){ #Prevents re-load
  ...
  rs_request(
    ...
    user = "vaneend",
    transfer = TRUE,
    path = "../data/Phenology_modelling/", #Store data set in folder
    verbose = TRUE
  )
}
```
...
```{r gdd example, warning=FALSE, fig.align='center', fig.cap="Figure 1: GDD as the cumulative temperature sum with blue representing cold days not contributing to accumulating energy and red warm days contributing to accumulating energy for leave out for the year 2010. Vertical line representing the date of leave out and the horizontal line the critical cumulative GDD value at this day."}
...
gdd_val <- harvard_filter[doy, "gdd"]
...
ggplot(data = harvard_filter) +
  ...
  #Limit x-axis and y-axis range
  xlim(1, 180) +
  ylim(0, 1000) +
  
  #Add visual day of leave out
  geom_vline(xintercept = doy) + #Day of leave out
  geom_hline(yintercept = gdd_val) +
  ...
  labs(
    ...
    subtitle = paste(
      c("Day of leave out: ", doy,
        "; Critical cumulative GDD value: ", gdd_val),
      collapse = "")
  ) +
...
To derive an optimal value for the critical value of the GDD (GDD_Crit) such that the plants start to leaf-out and for $t_{ref}$, one can calibrate the GDD model hyperparamters *GDD_Crit* and $t_{ref}$ with past observations of leaf-out dates. The resulting calibrated hyperparameters and the goodness of the result is strongly dependent on the calibration method and LOESS function used. In the book a *simulated annealing* (SA) method combined with root mean square error (RMSE) LOESS function was chosen. To compare the results, I have chosen to stick with the SA such that the result of my optimization of the GDD function can be compared with the result given in the book. However, for larger data sets one might consider using the Bayesian optimization method due to the fact that the latter method converges faster to an "optimal" solution. In addition it is worth mentioning at this point that the SA-method is a heuristic method. The problem with heuristic optimization models are that they do not always find *the* optimal solution. If *the* optimal solution wants to be found, one might consider using an alternative optimization tool.
...
Given the resulting data frame, we can now plot the observed values (phenology data set) with the predicted values. I added a dotted line, which should represent the ideal case, where the predicted values exactly match the observed values, as well as a solid line, which represent a linear fit through the plotted data (i.e. the mean modelled vs. observed case deviation).
...
```{r model evaluation, message=FALSE, fig.align='center', fig.cap="Figure 2: Scatterplot of observed and modelled DOY of leave out. A linear regression (solid line) is overplotted, including a confidential interval (grey area) and a ideal szenario of relation between observed and medelled DOY (dotted line)."}
ggplot(data = predictions) +
  ...
  labs(
    x = "Observation",
    y = "Prediction",
    title = "Scatterplot: Predicted vs. observed day of leave out",
    subtitle = paste(
      c("Regression function: f(x) = ", a, "x + ", b),
      collapse = "")
  ) +
  theme_bw()
```
...
r1 <- terra::rast(
  "../data/Phenology_modelling/MCD12Q2.061_2010/MCD12Q2.061_Greenup_0_doy2010001000000_aid0001.tif"
  )
...
```{r plot observed data, fig.align='center', fig.cap="Figure 3: Spacially observed date of leave out for the greater Boston (MA) region in the US in 2010 given the MCD12Q12 data set"}
p1 <- ggplot() +
...
```{r load daymet data}
r_temp <- terra::rast("../data/Phenology_modelling/DAYMET.004_2010/DAYMET.004_1km_aid0001.nc")
...
```{r plot predicted result, fig.align='center', fig.cap="Figure 4: Spatially modelled date of leave out for the greater Boston (MA) region in the US in 2010 given the formula in chapter 4 and the DAYMET data set."}
...
```{r plot difference, fig.align='center', fig.cap="Figure 5: Difference between modelled and observed date of leave out in the greater Boston (MA) region in the US in 2010, where red shows a earlier leave out observed than predicted and blue a later leave out observed than predicted."}
...
ggplot() +
  ...
  #Create title
  labs(
    title = "Day of leave-out 2010; Prediction - observation",
    subtitle = "Boston (MA); USA"
  ) +
  theme_bw()
```
...
```{r compare result, fig.align='center', fig.cap="Figure 6: Side-by-side comparison of left the observed date of leave out in 2010 and right the predicted date of leave out in 2010 in Boston (MA) in the US."}
